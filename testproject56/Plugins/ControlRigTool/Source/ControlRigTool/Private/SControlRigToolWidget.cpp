	...
	const FName& SpaceName, const TArray<FName>& ChainBones, const FReferenceSkeleton& RefSkel)
{
	...
	for (const FName& BoneName : ChainBones)
	{
		...
		{
			// 그 외 세컨더리: Box_Solid, 크기 0.2, Z축으로 이동 (버텍스 바깥으로)
			ControlSettings.ShapeName = FName(TEXT("Box_Solid"));
			// Z축 오프셋 (60%로 증가 - 더 멀리 떨어지게)
			float OriginalOffset = ShapeInfo.Offset.Size() * 0.5f + 8.0f;
			FVector ZOffset = FVector(0.0f, 0.0f, OriginalOffset * 0.6f);
			ShapeTransform.SetLocation(ZOffset);
...
void SControlRigToolWidget::CreateIKRigFromTemplate()
{
	...
	FSavePackageArgs SaveArgsIKRig;
	SaveArgsIKRig.TopLevelFlags = RF_Public | RF_Standalone;
	UPackage::SavePackage(Package, NewIKRig, *PackageFileName, SaveArgsIKRig);
...
void SControlRigToolWidget::CreateTPoseAnimSequence()
{
	...
	for (const auto& Mapping : JishukaBoneMapping)
	{
		...
		// 템플릿에서 Jishuka 본 이름으로 트랙 찾기
		const FBoneAnimationTrack* Track = DataModel->FindBoneTrackByName(JishukaBone);
		if (Track)
		{
			if (Track->InternalTrackData.RotKeys.Num() > 0)
			{
				// 첫 번째 프레임의 회전값 사용, UE5 표준 본 이름으로 저장
				FQuat4f RotKey = Track->InternalTrackData.RotKeys[0];
				TemplateRotations.Add(StandardBone, FQuat(RotKey));
				UE_LOG(LogTemp, Log, TEXT("[TPose] Found track: %s -> %s, RotKeys=%d"), *StandardBone.ToString(), *JishukaBone.ToString(), Track->InternalTrackData.RotKeys.Num());
			}
			else
			{
				UE_LOG(LogTemp, Warning, TEXT("[TPose] Track found but no RotKeys: %s"), *JishukaBone.ToString());
			}
		}
		else
		{
	...
	FSavePackageArgs SaveArgsAnim;
	SaveArgsAnim.TopLevelFlags = RF_Public | RF_Standalone;
	bool bSaved = UPackage::SavePackage(Package, AnimSequence, *PackageFileName, SaveArgsAnim);
...
void SControlRigToolWidget::CreateIKRetargeter()
{
	...
	FString PackageFileName = FPackageName::LongPackageNameToFilename(NewAssetPath, FPackageName::GetAssetPackageExtension());
	FSavePackageArgs SaveArgsRetargeter;
	SaveArgsRetargeter.TopLevelFlags = RF_Public | RF_Standalone;
	UPackage::SavePackage(Package, NewRetargeter, *PackageFileName, SaveArgsRetargeter);
...
bool SControlRigToolWidget::CreateKawaiiAnimBlueprint()
{
	...
	// 패키지 저장
	FString PackageFileName = FPackageName::LongPackageNameToFilename(PackagePath, FPackageName::GetAssetPackageExtension());
	FSavePackageArgs SaveArgsAnimBP;
	SaveArgsAnimBP.TopLevelFlags = RF_Public | RF_Standalone;
	UPackage::SavePackage(Package, AnimBP, *PackageFileName, SaveArgsAnimBP);
...
bool SControlRigToolWidget::CreatePhysicsAsset()
{
	...
	for (const FName& BoneName : PhysAssetMainBones)
	{
		...
		if (ChildIndices.Num() > 0)
		{
			...
			for (int32 ChildIndex : ChildIndices)
			{
				...
				TotalLength += ChildLocalPos.Size();
			}
			BoneLength = FMath::Max(TotalLength / ChildIndices.Num(), 5.0f);
		}
		else
		{
			// 자식 본이 없으면 부모 본까지의 거리 사용
			FVector BoneLocalPos = RefBonePose[BoneIndex].GetLocation();
			BoneLength = FMath::Max(BoneLocalPos.Size() * 0.5f, 5.0f);
		}
		
		// 반지름은 길이의 1/4 정도로 설정 (최소 2, 최대 15)
		BoneRadius = FMath::Clamp(BoneLength * 0.25f, 2.0f, 15.0f);
		
		// 캡슐 길이는 전체 길이에서 양쪽 반구 부분 빼기
		float CapsuleLength = FMath::Max(BoneLength - BoneRadius * 2.0f, 0.0f);
		...
		CapsuleElem.Center = FVector(BoneLength * 0.5f, 0.0f, 0.0f); // 본 방향으로 중앙
		CapsuleElem.Rotation = FRotator(0.0f, 0.0f, 90.0f); // X축 방향으로 회전
		...
		UE_LOG(LogTemp, Log, TEXT("[PhysicsAsset] Created body for %s: Length=%.1f, Radius=%.1f"), 
			*BoneName.ToString(), BoneLength, BoneRadius);
	}
	...
	FSavePackageArgs SaveArgsPhys;
	SaveArgsPhys.TopLevelFlags = RF_Public | RF_Standalone;
	UPackage::SavePackage(Package, PhysAsset, *PackageFilePath, SaveArgsPhys);
