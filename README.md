...
- ✅ **세컨더리 정리**: 
  - Space별 라벨 (예: "Spine Space", "Pelvis Space")
  - 체인별 가로 배치 (체인 끝나면 다음 줄로)
  - 예: `L_Cape_A01 L_Cape_A02 L_Cape_A03` (같은 줄)
- ✅ **FK/IK Switch 피커**:
  - 팔/다리 PV 피커 옆에 Switch 버튼 자동 생성
  - 클릭 시 FK/IK 전환 팝업 표시
...
- ✅ **피커 조작**:
  - 드래그: 피커 위치 이동
  - 코너 핸들 드래그: 피커 크기 조절 (중심 기준)
  - 다중 선택 시 그룹 전체 동시 리사이즈
- ✅ **다중 선택**:
  - 일반 클릭: 단일 선택
  - Shift + 클릭: 추가 선택
  - Ctrl + 클릭: 토글 선택
  - 드래그 박스: 범위 내 피커 일괄 선택
  - 빈 공간 클릭: 전체 해제
- ✅ **확장된 작업 영역**: 줌 아웃 시에도 피커 자유롭게 이동 가능
